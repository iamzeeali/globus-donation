...
import { getAllRationsByOrg } from "../../../_actions/deliveryAction";
import moment from "moment";
import PropTypes from "prop-types";
import { connect } from "react-redux";
import ReactToExcel from "react-html-table-to-excel";

const RationMaster = ({ getAllRationsByOrg, allRations, match }) => {
  useEffect(() => {
    getAllRationsByOrg(match.params.handle);
                  ...
                  <tbody>
                    {allRations.map((ration) => (
                      <tr key={ration._id}>
                        ...
                        <td>{!ration.state ? "NA" : ration.state.state}</td>
                        <td>{!ration.city ? "NA" : ration.city}</td>

                        <td>{!ration.user ? "NA" : ration.user.username}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
RationMaster.propTypes = {
  getAllRationsByOrg: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { getAllRationsByOrg })(RationMaster);
